...
import { isWorkflowRunning, setWorkflowActive, stopWorkflow } from "./state";
...
export async function welcomeAndRoute(): Promise<void> {
  try {
      ...
      while (true) {
        ...
        const result = await runMenu(
          ...
          [
            ...
            {
              label: "Story Teller (hear a past story)",
              action: async () => {},
            },
            {
    ...
    while (true) {
      ...
      const again = await runMenu(
        ...
        [
          ...
          {
            label: "Exit",
            action: async () => speak("Goodbye. See you later!"),
          },
        ],
        { timeoutMs: 60_000 },
      );
    ...
    if (error instanceof Error && error.message.includes("interrupted")) {
      activity("flow", "workflow:interrupted");
      console.log("Workflow was interrupted by hinge closure");
    } else {
      activity("flow", "workflow:error", { error: String(error) });
...
// Re-export the state functions for external use
export { isWorkflowRunning, stopWorkflow };
